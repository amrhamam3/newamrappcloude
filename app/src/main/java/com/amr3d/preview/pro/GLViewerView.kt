...
class GLViewerView(context: Context, attrs: AttributeSet? = null) : GLSurfaceView(context, attrs) {
    ...
    /** بتتنادى لما الـ View نفسها تتشال نهائيًا من شجرة الواجهة (زي إغلاق شاشة
     * الـ Viewer والرجوع لقايمة الملفات) — عكس onPause() العادي، هنا الـ EGL
     * context هيتدمر فعلاً، فلازم نحرر الـ VBOs وبرامج الـ Shader بأنفسنا الأول
     * (queueEvent بتنفذها على GL thread قبل ما الخيط يقفل) عشان منسيبش أي
     * مقابض GPU معلّقة في الـ Native Heap (كانت دي أكبر سبب لـ OutOfMemoryError
     * بعد فتح/قفل الـ Viewer عدة مرات وراء بعض في نفس الجلسة). */
    override fun onDetachedFromWindow() {
        queueEvent { stlRenderer.release() }
