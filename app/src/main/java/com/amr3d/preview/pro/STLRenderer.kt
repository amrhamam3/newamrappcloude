...
class STLRenderer : GLSurfaceView.Renderer {
    ...
    /** بتحرر كل موارد الـ GPU (الـ 3 VBOs + الـ 3 shader programs) نهائيًا، عكس
     * clearModel() اللي بتفضّي محتوى الـ VBOs بس (buffer orphaning) من غير ما
     * تلغيها كمقابض عشان تتعاد استخدامها بسرعة وقت تبديل الموديلات العادي.
     * لازم تتنادى من على الـ GL thread بس (عبر queueEvent من الـ View) قبل ما
     * الـ EGL context يتدمر نهائيًا مع الـ View نفسها (onDetachedFromWindow) —
     * مش وقت onPause() العادي اللي بيحافظ على كل حاجة عمدًا (preserveEGLContextOnPause).
     * من غير كده، المقابض دي كانت بتفضل "معلّقة" في الـ Native Heap لحد ما
     * الـ GC/driver يقرر يلمّها، وده كان بيسبب استهلاك ذاكرة متصاعد مع كل
     * فتح/قفل لشاشة الـ Viewer خلال نفس الجلسة. */
    fun release() {
        if (vboIds[0] != 0 || vboIds[1] != 0 || vboIds[2] != 0) {
            GLES20.glDeleteBuffers(vboIds.size, vboIds, 0)
            vboIds[0] = 0; vboIds[1] = 0; vboIds[2] = 0
        }
        if (meshProgram != 0) { GLES20.glDeleteProgram(meshProgram); meshProgram = 0 }
        if (lineProgram != 0) { GLES20.glDeleteProgram(lineProgram); lineProgram = 0 }
        if (shadowProgram != 0) { GLES20.glDeleteProgram(shadowProgram); shadowProgram = 0 }
        vboReady = false
        vertexBuffer = null; normalBuffer = null; wireframeBuffer = null
        vertexCountToDraw = 0
        wireframeVertexCount = 0
